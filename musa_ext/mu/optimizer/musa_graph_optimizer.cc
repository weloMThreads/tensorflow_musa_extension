...
NodeDef* AddFloatConcat3Node(GraphDef* graph, const string& name,
  ...
  (*node->mutable_attr())["T"].set_type(DT_FLOAT);
...
int OptimizeQkvDenseDw(GraphDef* graph) {
  ...
  for (auto& item : groups) {
    ...
    AddFloatConcat3Node(graph, dy_concat_name, nodes[0]->input(1),
    ...
    (*split->mutable_attr())["T"].set_type(DT_FLOAT);
...
int OptimizeCausalAttentionGrad(GraphDef* graph) {
  ...
  for (int i = 0; i < original_node_size; ++i) {
    ...
    if (!softmax_is_causal || ds_bmm == nullptr || ds_bmm->op() != "BatchMatMulV2" ||
        ds_bmm->input_size() != 2 ||
        !HasFloatTypeAttr(*ds_bmm) || get_bool_attr(*ds_bmm, "adj_x") ||
